...
function loginInterface() {
  ...
  rl.question('Username: ', (input1) => {
    rl.question('Password: ', (input2) => {
      var arrayLogin = [];
      let perintahLogin = 'SELECT * FROM users where username=?'
      db.all(perintahLogin, [input1], (err, rows) => {
        if (err) { return console.error(err.message); }
        else {
          arrayLogin.push(rows);
          if (arrayLogin[0][0]) {
            if (input2 == arrayLogin[0][0].password) {
              console.log(`Welcome, ${arrayLogin[0][0].username}. Your access level is ${arrayLogin[0][0].access.toUpperCase()}`);
              mainInterface();
            } else {
